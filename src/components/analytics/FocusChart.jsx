...
function FocusChart() {
    // focus sessions are stored as a total count
    // we simulate daily data using a stored daily log
    const sessions = Number(localStorage.getItem('focusSessions')) || 0

    // try to get daily focus log — stored as { "2026-06-06": 2 }
    ...
    return (
        <div className="dash-card analytics-chart-card">
            <div className="dash-card-lbl">Focus Sessions — Last 7 Days</div>
            <div className="focus-total-label">
                {sessions} total sessions · {sessions * 25} mins focused all time
            </div>
